...
Follow the `DECISION_REQUIRED`/`DECISION_RESULT` and `PERMISSION_REQUIRED`/`PERMISSION_RESULT` handoff patterns from `AGENTS.md`. Codex-config leaf workers must never interact with the user directly — always escalate to the caller.
