...
Read `RESEARCH-INSTRUCTIONS.md` alongside this file for sourcing, verification, and report-shape rules that apply to all UE research work.
...
- Do not recreate Claude transport layers such as runner scripts, temp-file polling, or user-input restart protocols.
...
- The canonical memory policy is `agent-rules/agent-memory-rule.md`.
- Do not write shared memory during normal execution.
- Child UE workers should return reusable knowledge in a `Reusable Learnings` section so the caller can curate it into `docs/` or `memories/`.
...
- Prefer `ue-research-director` when the question mixes Unreal concerns with non-UE libraries, tooling, or APIs.
...
- For Blueprint, DataAsset, StateTree, or widget-structure questions, use Blueprint/MCP extraction tools when available before guessing from memory.
